JMIntechEN16 : JMIntechControllers { // JMIntechEN16: A subclass of JMIntechControllers designed specifically for the "Intech Studio EN16" MIDI controller, which features 16 encoders and 16 buttons
    ...
    *new { |midiChannel=0, startCC=32, deviceOSCpath="/en16", oscServAddr="127.0.0.1", oscServPort=9000|
        this.classDeviceNumb = this.classDeviceNumb + 1; // Increment the classDeviceNumb to assign a unique number to this instance.
        ^super.new.init("Intech Studio EN16", "EN16", midiChannel, oscServAddr, oscServPort).initEN16(startCC, deviceOSCpath) // Call the superclass's init method to set up the device with its name, short name, MIDI channel, OSC server address, and port
    }
