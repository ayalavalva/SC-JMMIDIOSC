JMOSCManager { // A class designed to handle OSC (Open Sound Control) communication, enabling the sending of OSC messages from SuperCollider to other OSC-enabled devices or software.
    ...
    *new { |oscServAddr, oscServPort, postMIDIOSC|
        ^super.new.init(oscServAddr, oscServPort, postMIDIOSC);
    }

    // Initialization method: Configures the OSC client with the provided server address and port.
    init { |oscServAddr, oscServPort, postMIDIOSC|
    ...
    *getSharedInstance { |oscServAddr = "127.0.0.1", oscServPort = 9000, postMIDIOSC = true| // Default Open Stage Control OSC server IP address and port
        if(sharedInstance.isNil, { // Check if the sharedInstance is not initialized.
            sharedInstance = this.new(oscServAddr, oscServPort, postMIDIOSC); // Create a new instance with default or provided parameters.
        });
    ...
    send { |oscPath, args, forcePost = false|
        if(oscServerStatus) { 
            if (postMIDIOSC or: forcePost) {"Sending OSC Message to Address: % Value: %".format(oscPath, args).postln}; // Post the outgoing OSC message to the post window if either postMIDIOSC or forcePost is set to true.
